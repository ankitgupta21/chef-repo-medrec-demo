...
# BEGIN *** Quest Foglight integration *** BEGIN

export QUEST_PREINST_OPTS QUEST_AGENT_CONFIG QUEST_LOG_CONFIG PERFORMASURE_OPTIONS
QUEST_PREINST_OPTS=""
PERFORMASURE_OPTIONS=
QUEST_AGENT_CONFIG=""
QUEST_LOG_CONFIG=""
QUEST_INTEGRATIONID=medrec
QUEST_DEPLOYMENT_DIRECTORY="/opt/dsg/Foglight_Agent/Manager/agents/JavaEE"
if [ -f "$QUEST_DEPLOYMENT_DIRECTORY/integrate-Vann.sh" ]
then
QUEST_JAVA_ENV_OPTS=WEBLOGIC:SERVER:_DISABLE_ENV_OPT_
. "$QUEST_DEPLOYMENT_DIRECTORY/integrate-Vann.sh"
else
...
if [ "${WLS_REDIRECT_LOG}" = "" ] ; then
	...
	echo "${JAVA_HOME}/bin/java ${JAVA_VM} ${MEM_ARGS} -Dweblogic.Name=${SERVER_NAME} -Djava.security.policy=${WL_HOME}/server/lib/weblogic.policy ${JAVA_OPTIONS} ${PROXY_SETTINGS} ${PERFORMASURE_BOOTSTRAP:-"-Dquest.env.bootstrap_opt=unused"} ${PERFORMASURE_JAVA5:-"-Dquest.env.java5_opt=unused"} ${QUEST_AGENT_CONFIG_OPT:-"-Dquest.env.agent_config_opt=unused"} ${QUEST_LOG_CONFIG_OPT:-"-Dquest.env.log_config_opt=unused"} ${PERFORMASURE_OPTIONS} ${PERFORMASURE_BOOTAGENT} ${SERVER_CLASS}"
	${JAVA_HOME}/bin/java ${JAVA_VM} ${MEM_ARGS} -Dweblogic.Name=${SERVER_NAME} -Djava.security.policy=${WL_HOME}/server/lib/weblogic.policy ${JAVA_OPTIONS} ${PROXY_SETTINGS} ${PERFORMASURE_BOOTSTRAP:-"-Dquest.env.bootstrap_opt=unused"} ${PERFORMASURE_JAVA5:-"-Dquest.env.java5_opt=unused"} ${QUEST_AGENT_CONFIG_OPT:-"-Dquest.env.agent_config_opt=unused"} ${QUEST_LOG_CONFIG_OPT:-"-Dquest.env.log_config_opt=unused"} ${PERFORMASURE_OPTIONS} ${PERFORMASURE_BOOTAGENT} ${SERVER_CLASS}
else
	echo "Redirecting output from WLS window to ${WLS_REDIRECT_LOG}"
	${JAVA_HOME}/bin/java ${JAVA_VM} ${MEM_ARGS} -Dweblogic.Name=${SERVER_NAME} -Djava.security.policy=${WL_HOME}/server/lib/weblogic.policy ${JAVA_OPTIONS} ${PROXY_SETTINGS} ${PERFORMASURE_BOOTSTRAP:-"-Dquest.env.bootstrap_opt=unused"} ${PERFORMASURE_JAVA5:-"-Dquest.env.java5_opt=unused"} ${QUEST_AGENT_CONFIG_OPT:-"-Dquest.env.agent_config_opt=unused"} ${QUEST_LOG_CONFIG_OPT:-"-Dquest.env.log_config_opt=unused"} ${PERFORMASURE_OPTIONS} ${PERFORMASURE_BOOTAGENT} ${SERVER_CLASS}  >"${WLS_REDIRECT_LOG}" 2>&1 
fi
